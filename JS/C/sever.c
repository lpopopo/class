...
/************************************************************************************************************************
 ...
 struct sockaddr_in{
    ...
    unsigned char si_zero[8];  //填充0以保持与sockaddr结构的长度相同 以便于类型强制转换
 };
 ...
 5、int accept(int sockfd,struct sockaddr *cliaddr,socklen_t *addrlen)  //？？？第三个参数
 sockfd:
 ...
 *************************************************************************************************************************/
 int main(int argc, char *argv[])
 {
     ...
     bzero(&(server_addr.sin_zero), 8);  //置结构体内部数据为0，与memset不同的是memset返回一个指针而bzero不返回数据
     struct_len = sizeof(struct sockaddr_in); 
     fd = socket(AF_INET, SOCK_STREAM, 0);   //创建tcp协议的socket
     while(bind(fd, (struct sockaddr *)&server_addr, struct_len) == -1);   //不断尝试创建tcp连接的服务器端socket直至成功
     printf("Bind Success!\n");
     while(listen(fd, 10) == -1);   //不断尝试监听服务器端socket直至成功能够监听10个客户端sicket连接
     printf("Listening....\n");
     printf("Ready for Accept,Waitting...\n");
     new_fd = accept(fd, (struct sockaddr *)&client_addr, &struct_len);
     printf("Get the Client %d\n",new_fd);
     numbytes = send(new_fd,"Welcome to my server\n",21,0); 
     fp = fopen("./test.bmp" ,  "ab");
      while(   (numbytes = recv(new_fd, &buff, 1, 0)) > 0 )
     {
